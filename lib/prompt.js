...
export function buildUserPrompt({ tweet, analysis, language, tones, draftsPerTone, maxChars }) {
  ...
  // Length is a style rule, not just a cap. A 200-character reply reads as
  // generated no matter how good the language is: real people type a line or
  // two under someone else's post. State the target, not just the maximum, so
  // the model has something to aim at instead of something to stay under.
  const charNote = language === 'zh-CN'
    ? `Length: ${Math.floor(REPLY_LENGTH.target / 2)}-${Math.floor(REPLY_LENGTH.max / 2)} Chinese characters per draft, `
      + `ideally around ${Math.floor(REPLY_LENGTH.target / 2)}. Maximum ${Math.floor(REPLY_LENGTH.max / 2)} `
      + `(X weights CJK at 2 units each). One or two sentences.`
    : `Length: ${REPLY_LENGTH.target} characters is the target, ${REPLY_LENGTH.max} is the hard maximum. `
      + `Aim for roughly ${REPLY_WORDS.target} words. One sentence is usually enough, two is the ceiling. `
      + `A long reply reads as machine-written even when the wording is natural.`;

  const brevityNote = [
    '',
    'BREVITY — this is a reply under someone else\'s post, not a post of your own:',
    '- Get to the point in the first few words. No preamble, no setup.',
    '- Do not explain your reasoning. A reaction, an observation or a jab, then stop.',
    '- Never summarise the post back to its author. They wrote it; they know.',
    '- Do not add a closing line. Ending on a slightly abrupt note is correct here.',
    `- If a draft is over ${REPLY_LENGTH.max} characters or ${REPLY_WORDS.max} words, cut it. Do not trim words; delete a whole clause.`,
  ].join('\n');
...
export function buildRepairPrompt({ original, language, tone, violations, siblings = [] }) {
  ...
  if (violations.includes('too_long')) fixes.push(
    `- Too long. It must be at most ${REPLY_LENGTH.max} characters and ${REPLY_WORDS.max} words, `
    + `with ${REPLY_LENGTH.target} characters as the target. Do not shorten word by word: delete a whole clause `
    + 'or drop the closing sentence. Keep the sharpest part and end there.');
