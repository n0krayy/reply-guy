...
/**
 * Turns an opaque transport failure into something the user can act on.
 *
 * `Failed to fetch` is the browser's catch-all for two very different problems,
 * and they need different fixes:
 *   - the host is unreachable (typo, dead server, wrong port)
 *   - the host is up but refused the request cross-origin (CORS)
 * A Chromium extension cannot read the CORS headers to tell them apart, which
 * is why the old "check that the server is running" message sent people hunting
 * for a problem that was not there.
 */
function describeFetchFailure(err, base, timeoutMs) {
  if (err?.name === 'AbortError') {
    return `Timed out after ${timeoutMs / 1000}s. The endpoint accepted the connection but did not answer.`;
  }

  let host = base;
  try { host = new URL(base).host; } catch { /* keep raw base */ }

  // Precise causes are only visible outside the browser; use them when present.
  const cause = err?.cause?.code || err?.cause?.message;
  if (cause && /ENOTFOUND|EAI_AGAIN/.test(String(cause))) {
    return `${host} does not resolve. Check the base URL for typos.`;
  }
  if (cause && /ECONNREFUSED/.test(String(cause))) {
    return `Nothing is listening on ${host}. Check the port and that your server is running.`;
  }
  if (cause && /CERT|SSL|TLS/i.test(String(cause))) {
    return `TLS handshake with ${host} failed (${cause}).`;
  }

  return `Could not reach ${host}. Either the host is down or the URL is wrong, ` +
    `or the endpoint refuses browser requests (CORS). Check the base URL, and check ` +
    `that the endpoint allows requests from a Chrome extension.`;
}
...
export async function chatCompletion(cfg, req, { timeoutMs = 60000 } = {}) {
  ...
  try {
    ...
    clearTimeout(timer);
    throw new Error(describeFetchFailure(err, base, timeoutMs));
  }
  clearTimeout(timer);
